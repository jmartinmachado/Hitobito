...
class ModifiedImage{
	...
	public function resizeToFit($width, $height, $margins = false, $hexBckColor = '000000',$center=true) {
		...
		if($newW != $width || $newH != $height) {
			...
			if ($center){
				...
				$ox =0; //($newW > $width) ? floor(($newW - $width) / 2) : 0;
				$dy =0; //($newH < $height) ? floor(($height - $newH) / 2) : 0; 
				$oy =0; // ($newH > $height) ? floor(($newH - $width) / 2) : 0;
				$dx =0; // ($newW < $width) ? floor(($width - $newW) / 2) : 0;
			}
